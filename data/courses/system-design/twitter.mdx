...
Twitter is a social media service where users can read or post short messages (up to 280 characters) called "tweets". Twitter is available on the web and mobile.
...
$$
10\% \times 1 \space billion = 100 \space million/day
$$
...
We also know that around 10 percent of our daily messages (100 million) are media files per our requirements. If we assume each file is 50 KB on average, we will require 5 TB of object storage every day.

$$
1 \space billion \times 100 \space million \times 100 \space KB = 5 \space TB/day
$$
...
As our system is handling 5.1 TB of ingress every day, so we will a require minimum bandwidth of around 60 MB per second.

$$
\frac{5100 \space GB}{(24 \space hrs \times 3600 \space seconds)} = \sim 60 \space MB/second
$$
...
[https://drawsql.app/karanpratapsingh/diagrams/twitter/embed](https://drawsql.app/karanpratapsingh/diagrams/twitter/embed)
...
As the name suggests, this table will store tweets with properties such as `type` (text, image, video, etc.), `content`, etc. The tweet will also have a corresponding `userID`.

**favorites**

This table maps tweets with users for the favorite tweets functionality of our application.

**followers**

This table maps followers and [followees](https://en.wiktionary.org/wiki/followee) as users can follow each other (N:M relationship).
...
Tweets (`Tweet[]`): All the tweets for a given newsfeed.
...
This service handles user-related concerns such as user information.
...
Since our architecture is micro-services based, services will be communicating with each other as well. Generally, REST or HTTP performs well but we can further improve the performance using [gRPC](/courses/system-design/rest-graphql-grpc#grpc) which is more lightweight and efficient.
...
3. Use a ranking algorithm to rank the tweets based on relevance, time, engagement, etc.
4. Return the ranked tweets data in a paginated manner to the client.
...
Publishing is the step where the feed data is displaced according to each specific user. This can be quite a heavy operation, as the user may have millions of friends or followers. To deal with this, we have three different approaches:
...
In this model, once a user creates a tweet, it is "pushed" to all the followers immediately. This prevents the system from having to go through a user's entire followers list to check for updates on their posts published.

However, the downside of this approach is that it would result in an increase in the number of write operations on the database.
...
The hybrid model allows only users with a lesser number of followers to use the push model and for users with a higher number of followers, the pull model will be used.

### Ranking Algorithm

As we discussed, we will need a ranking algorithm to rank each tweet according to its relevance to each specific user.

For example, Facebook used to utilize an [EdgeRank](https://en.wikipedia.org/wiki/EdgeRank) algorithm, the rank of each feed item is described by:
...
Nowadays, algorithms are much more complex and ranking is done using machine learning models which can take thousands of factors into consideration for ranking.

### Retweets

Retweets are one of our extended requirements. To implement this feature we can simply create a new tweet with the user id of the user re-tweeting the original tweet and then modify the `type` enum and `content` property of the new tweet to link it with the original tweet.

For example, the `type` enum property can be of type tweet, similar to text, video, etc and `content` can be the id of the original tweet. Here the first row indicates the original tweet while the second row is how we can represent a re-tweet.
...
This is a very basic implementation, to improve this we can also create a separate table itself to store re-tweets.

### Search

Sometimes traditional DBMS are not performant enough, we need something which allows us to store, search, and analyze huge volumes of data quickly and in near real-time and give back answers in milliseconds. [Elasticsearch](https://www.elastic.co/) can help us with this use case.
...
We can cache the most frequently searched search queries, hashtags, and topics in the last `N` seconds and update them every `M` seconds using some sort of batch job mechanism. Our ranking algorithm can also be applied to the trending topics to give them more weight and personalize them for the current user.

### Notifications

Push notifications are an integral part of any social media platform. We can use a message queue or a message broker such as [Apache Kafka](https://kafka.apache.org/) with the notification service to dispatch requests to Firebase Cloud Messaging (FCM) or Apple Push Notification Service (APNS) which will handle the delivery of the push notifications to user devices.

_For more details, refer to the [Whatsapp system design](/courses/system-design/whatsapp#notfications) tutorial where we discuss push notifications._
...
For suggestions, we can build a social graph for every user. Each node in the graph will represent a user and a directional edge will represent followers and followees. After that, we can traverse the followers of a user to find and suggest a mutual friend. This would require a graph database such as [Neo4j](https://neo4j.com/) and [ArangoDB](https://www.arangodb.com/).

This is a pretty simple algorithm, To improve our suggestion functionality, we will need to incorporate a recommendation model which uses machine learning as part of our algorithm.
