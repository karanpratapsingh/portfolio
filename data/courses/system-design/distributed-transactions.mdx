...
The two-phase commit protocol (2PC) is a distributed algorithm that coordinates all the processes that participate in a distributed transaction on whether to commit or abort (roll back) the transaction.
...
Following problems may arise in two-phase commit protocol:
...
Three-phase commit (3PC) is an extension of two-phase commit where the commit phase is split into two phases. This helps with the blocking problem that occurs in the two-phase commit protocol.
...
Coordinator issues the pre-commit message and all the participating nodes must acknowledge it. If a participant fails to receive this message in time, then it aborts.

**Commit phase**

This step is also similar to the two-phase commit protocol.

### Why is pre-commit phase helpful?

The pre-commit phase which is the second phase accomplishes
