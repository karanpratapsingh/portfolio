...
A monolithic is a self-contained and independent application. It is built as a single unit and is responsible for not just a particular task, but can perform every step needed to satisfy a business need.
...
- Maintenance is complex as the codebase grows.
...
This approach reduces the dependencies of a module in such as way that we can enhance/change a module without it effecting other modules. When done right, this can be really beneficial in the long term as it will reduces complexity that comes with maintaining a monolith as the system grows.
...
Each service is a separate codebase, which can be managed by a small development team. Services can be deployed independently and a team can update an existing service without rebuilding and redeploying the entire application.
...
Microservices architecture style has the following characteristics:

- **Loosely coupled:** Services should be loosely coupled so that they can be independently deployed and scaled. This will lead to the decentralization of development teams and will lead them to develop and deploy faster with minimal constraints and operational dependencies.
- **Small but focused:** It's about scope and responsibilities and not size, a service should be focused on a specific problem. Basically, _It does one thing and does it well_. Ideally, they can be independent of the underlying architecture.
- **Built for businesses:** The microservices architecture is usually organized around business capabilities and priorities.
- **Resilience & Fault tolerance:** Services should be designed in such a way that they still function in case of failure or errors as in environments with independently deployable services failure tolerance is of the highest importance.
...
- Improves fault and data isolation.
- Better scalability as each service can be scaled independently.

### Disadvantages

Microservices architecture brings its own sets of challenges:

- Complexity of a distributed system.
- Testing is more difficult.
- Expensive to maintain (individual servers, databases, etc.).
- Inter-service communication has its own issues.
...
- Forgetting lack of clear ownership
- Lack of idempotency.
...
- Sharing same resources such as databases.
...
You might have seen _Service Oriented Architecture (SOA)_ mentioned around the internet, sometimes even interchangeably, but they are different from each other and the main distinction between the two approaches comes down to _scope_.
