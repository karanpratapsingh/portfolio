...
In CQRS, a _command_ is an instruction, a directive to perform a specific task. It is an intention to change something and doesn't return a value, only an indication of success or failure. And, a \*query\* is a request for information that doesn't change the system's state or cause any side effects that change the state.
...
The core principle of CQRS is the separation of commands and queries. They perform fundamentally different roles within a system, and separating them means that each can be optimized as needed, which can be hugely beneficial for distributed systems.
