...
date: '2022-10-01'
draft: false
summary: 'In this tutorial we will discuss system design problem URL Shortener'
images: ['/static/courses/system-design/chapter-V/url-shortener/banner.png']
authors: ['default']
---

Let's design a URL shortener service, similar to services like `bit.ly`, `goo.gl`, `tinyURL`.
...
URL shortener saves space in general when we are sharing URLs. Users are also less likely to mistype shorter URLs. Moreover, we can also optimize links across devices, this allows us to track individual links and analytics.

## Requirements

A URL shortening system should meet the following requirements:
...
Let's start with the estimation and constraints part.

_Note: Make sure to double-check with your interviewer regarding any assumptions, here we are using numbers like 100 million just to simplify the math._

### Traffic

This will be a very read-heavy system, so let's assume a `100:1` read/write ratio with 100 million links generated per month.
...
$$
1 \times 100 \space Million = 1 \space Million/month
$$
...
$$
100 \times 40 \space URLs/second = 4000 \space URLs/second
$$
...
$$
100M \times 10\space Years \times 12 \space Months = 12 \space Billion
$$
...
$$
20\% \times 350 \space Million \times 500 \space bytes = 35 \space GB/day
$$

## Estimates

Here are our high-level estimates:

| Writes (New URLs)    | 40/s      |
| -------------------- | --------- |
| Reads (Redirection)  | 4K/s      |
| Bandwidth (Incoming) | 20KB/s    |
| Bandwidth (Outgoing) | 2MB/s     |
| Storage (10 years)   | 6TB       |
| Memory (Caching)     | ~35GB/day |
...
API Key (`string`): API key provided to the user.
...
Short Url (`string`): Short URL to be retrieved.

**Returns**

Result (`boolean`): Represents successful deletion of the URL.

### Why do we need an API key?

As you must've noticed, we're using an API key to prevent abuse of our services. Using this API key we can limit the users to a certain number of requests per minute or hour. This is quite a standard practice for developer APIs and should cover our extended requirement.
...
<iframe
  ...
  title='URL Shortener'
  frameBorder='0'
  src='https://drawsql.app/karanpratapsingh/diagrams/url-shortener/embed'
></iframe>

Initially, we can get started with two tables:

- **users**: Stores user's details such as name, email, created at, etc.
- **urls**: Contains the new short URL's properties such as expiration, hash, original URL, and the user who created the short URL.

We can also use the `hash` column as an [index](/courses/system-design/indexes) to improve query performance.
...
Now, let us discuss the high-level design of our system.
...
The easiest way to solve this would be to store keys in two tables. As soon as a key is used, we move it to a separate table with appropriate locking in place. Also, to improve reads, we can also keep some keys in memory.
...
![basic-url-shortener](/static/courses/system-design/chapter-V/url-shortener/basic-url-shortener.png)
...
![advanced-url-shortener](/static/courses/system-design/chapter-V/url-shortener/advanced-url-shortener.png)

Let us identify and resolve bottlenecks such as single points of failure in our design:

- "What if the API service of Key Generation Service crashes?"
...
- Multiple instances and replicas for our cache.
