...
_Generally, static files such as HTML/CSS/JS, photos, and videos are served from CDN._

## Why use a CDN?

CDN increases content availability and redundancy while reducing bandwidth costs and improving website security. Serving content from CDNs can significantly improve performance as users receive content from data centers close to them and our servers do not have to serve requests that the CDN fulfills.
...
For example, when someone in the UK requests your website which might be hosted in the USA, will be served from the closest edge location such as the London edge location. This is much quicker than having the visitor make a complete request to the origin server which will increase the latency.

## Types

Here are the types of CDNs:
...
- **Location:** If most of our audience is located in a country where the CDN has no servers, the data on your website may have to travel further than without using any CDN.
