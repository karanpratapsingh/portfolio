...
Single Sign-On (SSO) is an authentication process in which a user is provided access to multiple applications and/or websites by using only a single set of login credentials. This prevents the need for the user to log separately into the different applications.
...
An identity broker acts as an intermediary that connects multiple service providers with various different identity providers. Using Identity Broker, we can perform single sign-on over any applications without the hassle about the protocol it follows.
...
There are many differences between SAML and OAuth and OIDC. SAML uses XML to pass messages, and OAuth and OIDC uses JSON. OAuth provides a simpler mobile experience, while SAML is geared towards enterprise security.

OAuth and OIDC uses RESTful communication extensively, which is why mobile applications, modern web applications devices find OAuth and OIDC a better experience for the user. SAML, on the other hand, drops a session cookie in a browser that allows a user to access certain web pages great for short-lived workloads.
...
- Ease of access without having to go through lengthy authorization process.
- Enforced security and compliance to protect the sensitive data.
...
- Single Password Vulnerability, If the main SSO password get's compromised, all the supported applications get compromised.
