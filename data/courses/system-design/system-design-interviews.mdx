...
System design is a very extensive topic and system design interviews are designed to evaluate our capability to produce technical solutions to abstract problems, as such, they're not designed with a specific correct answer. The unique aspect of system design interviews is the two-way nature between candidate and interviewer.
...
These are the requirements that the end user specifically demands as basic functionalities that the system should offer. All these functionalities need to be necessarily incorporated into the system as a part of the contract.
...
These can be "nice to have" requirements that might be out of the scope of the system.
...
Once we have the estimations, we can start with defining the database schema. As doing so in the early stages of the interview would help us to understand the data flow which is the core of every system. In this step, we basically define all the entities and relationships between them.
...
Now we have established our API and data model design, it's time to identify system components (such as Load Balancers, API Gateway, etc.) that are needed to solve our problem and draft the first design of our system.
...
Now it's time to go into detail about the major components of the system we designed. As always discuss with the interviewer which component may need further discussion.
...
Make sure to read the engineering blog of the company you're interviewing with. This will help you get a sense of what technology stack they're using and which problems are important to their engineering team.
