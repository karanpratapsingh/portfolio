...
1. Go is smart enough to interpret our function call correctly and hence, pointer receiver method calls are just syntactic sugar provided by Go for convenience.

```go
(&c).UpdateName(...)
```

1. We can omit the variable part of the receiver as well if we're not using it

```go
func (Car) UpdateName(...) {}
```

1. Methods are not limited to structs but can also be used with non-struct types as well
...
### W**hy methods instead of functions**
