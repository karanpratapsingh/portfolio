...
_Feel free to use any other code editor you prefer_
...
In Go, string is a sequence of bytes.

They are declared either using double quotes or back ticks which can span multiple lines
...
Go has several built-in integer types of varying size for storing signed and unsigned integers
...
![numeric-operators](/static/courses/go/chapter-I/variables-and-data-types/numeric-operators.png)
...
Now let's discuss zero values. So in Go any variable declared without an explicit initial value are given their _zero value_. For example, let's declare some variables and see
...
This is great, but what are those percent symbols in our `Printf` function? As you've already guessed, they are used for formatting and we will learn about them in the next video.
...
func main() {
	...
	fmt.Printf("%T - %s", str, str) // Output: I am an alias
}
```
...
It's bit confusing at first, hopefully, this example will make things clear.
...
As we can see, we cannot use the defined type interchangeably with the underlying type, unlike _alias types._

Well, this is pretty much it for variables and data types in Go. I'll see you in the next one.

# String Formatting

In this video, we will learn about string formatting or sometimes also known as templating.
...
Next, we have `Printf` also known as *“Print Formatter”, w*hich allows us to format numbers, strings, booleans, and much more.
...
So these are called annotation verbs and they tell the function how to format the arguments. We can control things like width, types, and precision with these and there are lots of them. Here's a [cheatsheet](https://pkg.go.dev/fmt)
...
Also, to add an actual percent sign we will need to _escape it._
...
And if we run this, we'll see that after the first case match the switch statement continues to the next case because of the `fallthrough` keyword
...
Lastly, If we omit the loop condition it loops forever, so an infinite loop can be compactly expressed. This is also known as forever loop
...
```bash
$ go mod init **[github.com/e](http://github.com/karanpratapsingh/go-course)xample/repo
```
...
Next, we have `go env` which simply prints all the go environment information, we'll learn about some of these build-time variable in the next video.

Lastly, we have, `go doc` which shows documentation for package or symbol, here's an example for format package
...
This is pretty much it on go commands, feel free to experiment with them! And I'll see you in the next video.
...
This helps us to produce [statically linked binary](https://en.wikipedia.org/wiki/Static_build) that works without any external dependencies.
...
Let's cleanup our example our bit and see how we can access individual fields
...
So this wraps up our discussion on methods, see you in the next video
...
We can also let compiler infer the length of the array by using `...` ellipses instead of the length
...
Well, this wraps up our discussion on maps, see you in the next video!
...
For example, now we can check explicitly which error occurred using the `[errors.Is](http://errors.Is)` function
...
Perfect, this wraps us our discussion on generics. I'll see you in the next video!
...
The final outcome of a concurrent program is same as that of a program that has been executed sequentially.
...
[Communicating Sequential Processes](https://dl.acm.org/doi/10.1145/359576.359585) (CSP) is a model put forth by Tony Hoare in 1978 which describes interactions between concurrent processes. It made a breakthrough in Computer Science, especially in the field of concurrency.
