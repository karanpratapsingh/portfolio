...
So this is it for functions in Go.

# Modules

Now, we will learn about modules.
...
Next, we will learn about multi-module workspaces that were introduced in Go 1.18
...
So, this is pretty much it for workspaces in Go.
...
Lastly, I will mention that `panic` and `recover` can be considered similar to the `try/catch` idiom in other languages. But one important factor is that we should avoid panic and recover and use [errors](/courses/go/error-handling) when possible.
...
I hope this tutorial helped you understand how to use `panic` and `recover` in Go.
